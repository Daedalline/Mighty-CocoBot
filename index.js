...
async function main(){
    ...
    let weeklyReminder = schedule.scheduleJob('00,15,30,45 * * * * *', printWeeklyReminderMessage); // fires every day, at 2:00:00 PM EST, except Monday
...
async function printWeeklyReminderMessage() {
    ...
    for (var map in Maps.Leaderboards) {
        if (Maps.Leaderboards[map].startsWith("Weekly")) {
			console.log("MAP: " + map);
			let parsedMap = map.replace("Weekly: ", "").replace(" (Race Mode)", "");
			console.log("PARSED: " + parsedMap);
