...
fs.access("ar_data.json", fs.F_OK, (err) => {
    if (err) {
        if(err.code == "ENOENT"){
            fs.writeFile("ar_data.json", data, (err) => {
                if (err) throw err;
                console.log("Created ar_data.json as it didnt exist")
                return
            })
        }else{
            console.error(err)
        ...
        console.log("ar_data.json already exists")
    }
})
