...
module.exports.run = async(interaction, config, maps, client) => {
    ...
    if(interaction.channel.id != config.CourseCommunityChannelID && !member.permissions.has("MANAGE_MESSAGES")){
        interaction.reply({ephemeral: true, content: "You are not allowed to do that in this channel"})
        return
    }
    await interaction.deferReply();
    
    let rawdata = await fs.readFileSync('community_pro_challenge_data.json');
